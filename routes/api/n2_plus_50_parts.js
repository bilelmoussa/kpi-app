...
router.put('/updateparts',(req, res, next)=>{
  ...
  passport.authenticate('jwt', {session: false}, function(err, user){
    update_auth(req, res, next,err,user,N2_plus_50_Part, query);
  })(req, res, next)
});
...
router.get('/years', (req, res, next)=>{
	passport.authenticate('jwt', {session: false}, function(err, user){
		...
		if(user){
			N2_plus_50_Part.get_years((err, years)=>{
				if(err){
...
router.get('/months/:year', (req, res, next)=>{
	passport.authenticate('jwt', {session: false}, function(err, user){
		...
		if(user){
			...
			N2_plus_50_Part.get_months(year, (err, months)=>{
				if(err){
					res.status(400).json({errors: err});
				}
				if(months){
...
router.get('/weeks/:year/:month', (req, res, next)=>{
	passport.authenticate('jwt', {session: false}, function(err, user){
		...
		if(user){
			...
			N2_plus_50_Part.get_weeks(year, month, (err, weeks)=>{
				if(err){
					res.status(400).json({errors: err});
				}
				if(weeks){
...
router.get('/month_chart/:year/:month', (req, res, next)=>{
	passport.authenticate('jwt', {session: false}, function(err, user){
		...
		if(user){
			let year = parseInt(req.params.year);
			let month = parseInt(req.params.month);
			N2_plus_50_Part.getMonthChartValues(year, month, (err, data)=>{
				if(err){
					res.status(400).json({errors: err});
				}
				if(data){
...
router.get('/year_chart/:year', (req, res, next)=>{
	passport.authenticate('jwt', {session: false}, function(err, user){
		...
		if(user){
			let year = parseInt(req.params.year);
			N2_plus_50_Part.getYearChartValues(year, (err, data)=>{
				if(err){
					res.status(400).json({errors: err});
				}
				if(data){
...
router.get('/week_table/:year/:month/:week', (req, res, next)=>{
	passport.authenticate('jwt', {session: false}, function(err, user){
		...
		if(user){
			...
			let month = parseInt(req.params.month);
			let week = parseInt(req.params.week);
			N2_plus_50_Part.getWeekTableValues(year, month, week, (err, data)=>{
				if(err){
					res.status(400).json({errors: err});
				}
				if(data){
