...
//get all parts worked with n2_plus_150

/*
router.get('/findparts',(req, res, next)=>{
  passport.authenticate('jwt', {session: false}, function(err, user){
    auth(req, res, next,err,user,N2_plus_150_Part);
  })(req, res, next)
});
*/

//create new part to n2_plus_150
router.post('/saveparts',  (req, res, next)=>{
  passport.authenticate('jwt', {session: false}, function(err, user){
    write_auth(req, res, next,err,user,N2_plus_150_Part,options);
  })(req, res, next)
});

//update part to N2_plus_150
router.put('/updateparts',(req, res, next)=>{
  let query = req.body.query;
  passport.authenticate('jwt', {session: false}, function(err, user){
    update_auth(req, res, next,err,user,N2_plus_150_Part, query);
  })(req, res, next)
});


//delete part from N2_plus_150
router.delete('/deleteparts',(req, res, next)=>{
  passport.authenticate('jwt', {session: false}, function(err, user){
    delete_auth(req, res, next,err,user,N2_plus_150_Part);
  })(req, res, next)
});
...
router.get('/findparts', (req, res, next)=>{
	passport.authenticate('jwt', {session: false}, function(err, user){
		...
		if(user){
			N2_plus_150_Part.get_by_date((err, parts)=>{
				if(err){
...
router.get('/years', (req, res, next)=>{
	passport.authenticate('jwt', {session: false}, function(err, user){
		...
		if (!user) { return res.json('Unauthorised user not found !'); }
		if(user){
			N2_plus_150_Part.get_years((err, years)=>{
				if(err){
					res.status(400).json({errors: err});
				}
				if(years){
...
router.get('/months/:year', (req, res, next)=>{
	passport.authenticate('jwt', {session: false}, function(err, user){
		...
		if(user){
			...
			N2_plus_150_Part.get_months(year, (err, months)=>{
				if(err){
					res.status(400).json({errors: err});
				}
				if(months){
...
router.get('/weeks/:year/:month', (req, res, next)=>{
	passport.authenticate('jwt', {session: false}, function(err, user){
		...
		if(user){
			...
			N2_plus_150_Part.get_weeks(year, month, (err, weeks)=>{
				if(err){
					res.status(400).json({errors: err});
				}
				if(weeks){
...
router.get('/week_chart/:year/:month/:week', (req, res, next)=>{
	passport.authenticate('jwt', {session: false}, function(err, user){
		...
		if(user){
			...
			N2_plus_150_Part.getWeekChartValues(year, month, week, (err, data)=>{
				if(err){
					res.status(400).json({errors: err});
				}
				if(data){
...
router.get('/month_chart/:year/:month', (req, res, next)=>{
	passport.authenticate('jwt', {session: false}, function(err, user){
		...
		if(user){
			let year = parseInt(req.params.year);
			let month = parseInt(req.params.month);
			N2_plus_150_Part.getMonthChartValues(year, month, (err, data)=>{
				if(err){
					res.status(400).json({errors: err});
				}
				if(data){
...
router.get('/year_chart/:year', (req, res, next)=>{
	passport.authenticate('jwt', {session: false}, function(err, user){
		...
		if(user){
			let year = parseInt(req.params.year);
			N2_plus_150_Part.getYearChartValues(year, (err, data)=>{
				if(err){
					res.status(400).json({errors: err});
				}
				if(data){
